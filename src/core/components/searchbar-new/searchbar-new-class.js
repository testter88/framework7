...
class SearchbarNew extends FrameworkClass {
    ...
    if ($el.parents('.page').length > 0) {
      ...
      if (!$pageEl.length) {
        ...
        if (
          ...
          $currentPageEl[0].f7Page.navbarNewEl === $navbarEl[0]
        ) {
      ...
      if (sb.enabled) {
        ...
        if (sb.expandable) {
          sb.$el
            .parents('.navbar-new')
            .removeClass(
              'with-searchbar-new-expandable-enabled with-searchbar-new-expandable-enabled-no-transition',
            );
        }
      }
    }
    function onPageBeforeIn() {
      ...
      if (sb.enabled) {
        ...
        if (sb.expandable) {
          sb.$el
            .parents('.navbar-new')
            .addClass('with-searchbar-new-expandable-enabled-no-transition');
        }
      }
    }
  ...
  enable(setFocus) {
    ...
    function enable() {
      if (
        ...
        !sb.$el.hasClass('searchbar-new-enabled') &&
        !sb.query
      ) {
        sb.backdropShow();
      }
      ...
      if (sb.expandable) {
        ...
        if ($navbarEl.hasClass('navbar-new-large') && sb.$pageEl) {
          ...
          if (
            $navbarEl.hasClass('navbar-new-large') &&
            $navbarEl.hasClass('navbar-new-large-collapsed') &&
            $titleLargeEl.length &&
    ...
    if (isIos) {
      ...
      if (app.theme === 'md' && sb.expandable) {
        sb.$el
          .parents('.page, .view, .navbar-new-inner, .navbar-new')
  ...
  disable() {
    ...
    sb.$el.removeClass(
      'searchbar-new-enabled searchbar-new-focused searchbar-new-enabled-no-disable-button',
    );
    if (sb.expandable) {
      const $navbarEl = sb.$el.parents('.navbar-new');
      ...
      if ($navbarEl.hasClass('navbar-new-large') && $pageContentEl.length) {
        ...
        $pageContentEl
          .removeClass('with-searchbar-new-expandable-enabled')
          .addClass('with-searchbar-new-expandable-closing');
      }
      if (app.theme === 'md' && $navbarEl.length) {
        $navbarEl
          .removeClass(
  ...
  search(query, internal) {
    ...
    if (
      ($searchContainer && $searchContainer.length && $el.hasClass('searchbar-new-enabled')) ||
      (sb.params.customSearch && $el.hasClass('searchbar-new-enabled'))
    ) {
      if (query.length === 0) {
        sb.backdropShow();
      } else {
    ...
    if (sb.params.customSearch) {
      $el.trigger('searchbar-new:search', { query, previousQuery: sb.previousQuery });
    ...
    if (isVirtualList) {
      ...
      $searchContainer
        ...
        .removeClass('hidden-by-searchbar-new')
        .each((itemEl) => {
          const $itemEl = $(itemEl);
          let compareWithText = [];
      ...
      if (sb.params.hideGroupTitles) {
        $searchContainer.find(sb.params.searchGroupTitle).each((titleEl) => {
          ...
          if (hide && !ignore) $titleEl.addClass('hidden-by-searchbar-new');
          else $titleEl.removeClass('hidden-by-searchbar-new');
        });
      }
    ...
    $el.trigger('searchbar-new:search', { query, previousQuery: sb.previousQuery, foundItems });
