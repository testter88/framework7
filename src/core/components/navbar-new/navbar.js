...
const Navbar = {
  size(el) {
    ...
    const needCenterTitle =
      $innerEl.hasClass('navbar-new-inner-centered-title') ||
      navbarNewParams[`${app.theme}CenterTitle`];
  ...
  show(el = '.navbar-new-hidden', animate = true) {
    const app = this;
    const $el = $(el);
    if (!$el.length) return;
    if (!$el.hasClass('navbar-new-hidden')) return;
  ...
  collapseLargeTitle(navbarEl) {
    ...
    $pageEl
      .eq(0)
      .addClass('page-with-navbar-new-large-collapsed')
  ...
  expandLargeTitle(navbarEl) {
    ...
    $pageEl
      .eq(0)
      .removeClass('page-with-navbar-new-large-collapsed')
  ...
  initNavbarOnScroll(pageEl, navbarEl, needHide, needCollapse, needTransparent) {
    ...
    const snapPageScrollToTransparentNavbar =
      app.params.navbarNew.snapPageScrollToTransparentNavbar;
    ...
    function handleLargeNavbarCollapse(pageContentEl) {
      ...
      const isLargeTransparent =
        $navbarEl.hasClass('navbar-new-large-transparent') ||
        ($navbarEl.hasClass('navbar-new-large') && $navbarEl.hasClass('navbar-new-transparent'));
...
export default {
  ...
  on: {
    ...
    pageInit(page) {
      ...
      if (
        app.params.navbarNew.hideOnPageScroll ||
        page.$el.find('.hide-navbar-new-on-scroll').length ||
        page.$el.hasClass('hide-navbar-new-on-scroll') ||
        page.$el.find('.hide-bars-on-scroll').length ||
        page.$el.hasClass('hide-bars-on-scroll')
      ) {
        if (
          page.$el.find('.keep-navbar-new-on-scroll').length ||
          page.$el.hasClass('keep-navbar-new-on-scroll') ||
      ...
      if (needCollapseOnScrollHandler || needHideOnScrollHandler || needTransparentOnScroll) {
        app.navbarNew.initNavbarOnScroll(
          page.el,
          $navbarEl[0],
