<template>
  <component :is="tag" ref="elRef" :class="classes" @submit="onSubmit">
    ...
    <div class="searchbar-new-inner">
      <slot name="inner-start" />
      <div class="searchbar-new-input-wrap">
        ...
        <input
          ...
          @blur="onBlur"
        />
        <i class="searchbar-new-icon" />
        <span v-if="clearButton" class="input-clear-button" @click="onClearButtonClick" />
        <slot name="input-wrap-end" />
      </div>
      <span v-if="disableButton" class="searchbar-new-disable-button" @click="onDisableButtonClick">
...
export default {
  ...
  emits: [
    ...
    'searchbar-new:clear',
    'searchbar-new:enable',
    'searchbar-new:disable',
    'update:value',
  ],
  setup(props, { emit }) {
